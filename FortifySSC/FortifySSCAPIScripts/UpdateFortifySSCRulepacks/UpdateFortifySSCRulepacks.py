...
# Checks if the log directory exists, if not it creates it
if log_directory:
    os.makedirs(log_directory, exist_ok=True)

# Configure logging
logging.basicConfig(filename=fortify_ssc_update_rulepacks_log_file, level=logging.INFO, format="%(asctime)s - %(levelname)s - %(message)s")
...
def create_fortify_ssc_token(fortify_token_type, fortify_ssc_api_url, fortify_ssc_user, fortify_ssc_password, fortify_ssc_api_request_headers):
    ...
    # Attempts to create a Fortify SSC Token with the specified type
    try:
        # Sends the API request to create the Token
        fortify_ssc_token_creation_response = requests.post(f"{fortify_ssc_api_url}/tokens", auth=HTTPBasicAuth(fortify_ssc_user, fortify_ssc_password), headers=fortify_ssc_api_request_headers, data=json.dumps(body), verify=False, timeout=900)
        ...
        if fortify_ssc_token_creation_response.status_code == 201:
            ...
            print(colored(f"{fortify_token_type} was successfully created.", "green"))

            # Logs the token creation
            logging.info(f"{fortify_token_type} was successfully created.")
            
            # Returns the token and token id
            ...
            print(colored(f"The status code from the request of Fortify SSC token creation is: " f"{fortify_ssc_token_creation_response.status_code}", "red"))
            ...
            # Prints the failure
            print(colored(f"Failed to create the token. " f"Status code: {fortify_ssc_token_creation_response.status_code} " f"- {fortify_ssc_token_creation_response.text}\n", "red"))

            # Logs the failure
            logging.error(f"Failed to create the token. " f"Status code: {fortify_ssc_token_creation_response.status_code} " f"- {fortify_ssc_token_creation_response.text}")

            # Returns None for both token and token id in case of failure
            return None, None

    # Handles any exceptions that occur during the request
    except requests.exceptions.RequestException as e:
        print(colored(f"[ERROR] Could not create Fortify SSC token: {e}", "red"))

        logging.error(f"[ERROR] Could not create Fortify SSC token: {e}")

        # Returns None for both token and token id in case of exception
        return None, None


# Function to update Fortify SSC Rulepacks
def update_fortify_ssc_rulepacks(fortify_ssc_api_url, fortify_ssc_api_request_headers):
    print(colored("Updating Fortify SSC Rulepacks...", "yellow"))
    ...
    # Attempts to update the Fortify SSC Rulepacks
    try:
        # Sends the GET request to the updateRulepacks endpoint
        fortify_ssc_update_rulepacks_response = requests.get(f"{fortify_ssc_api_url}/updateRulepacks", headers=fortify_ssc_api_request_headers, verify=False, timeout=900)

        # Checks if the request was successful
        if fortify_ssc_update_rulepacks_response.status_code == 200:
            # Prints a message that indicates that the request was successful
            print(colored("API Request was successful!", "green"))
            ...
            try:
                ...
                # Logs the response
                logging.info("Result from the updateRulepacks query: %s", json.dumps(response_json, separators=(",", ":")))

            # Handles any ValueError that occurs if the response is not JSON
            except ValueError:
                # Handles responses that are not JSON
                print(fortify_ssc_update_rulepacks_response.text)

                logging.info("Result from the updateRulepacks query: %s", fortify_ssc_update_rulepacks_response.text)

            print("")

            print(colored("Fortify SSC Rulepacks update request completed successfully!", "green"))

            logging.info("Fortify SSC Rulepacks update request completed successfully!")

            # Returns True to indicate that the update was successful
            return True

        # If the request was not successful, it prints an error message and logs the failure
        else:
            # Prints a message that indicates that the request was unsuccessful
            print(colored("API Request has failed!", "red"))
            ...
            # Prints the status code
            print(colored(f"The status code from the Fortify SSC updateRulepacks request is: " f"{fortify_ssc_update_rulepacks_response.status_code}", "red"))
            ...
            # Attempts to print the API response as JSON
            try:
                print(json.dumps(fortify_ssc_update_rulepacks_response.json(), indent=4))
            ...
            # Logs the failure
            logging.error(f"Failed to update Fortify SSC Rulepacks. " f"Status code: {fortify_ssc_update_rulepacks_response.status_code} " f"- {fortify_ssc_update_rulepacks_response.text}")

            # Returns False to indicate that the update was unsuccessful
            return False
...
def delete_fortify_ssc_token(fortify_ssc_token_id, fortify_ssc_api_url, fortify_ssc_user, fortify_ssc_password, fortify_ssc_api_request_headers):
    # Attempts to delete the Fortify SSC Token with the specified token id
    try:
        # Sends the API request to delete the Token created before
        fortify_ssc_token_deletion_response = requests.delete(f"{fortify_ssc_api_url}/tokens/{fortify_ssc_token_id}", auth=HTTPBasicAuth(fortify_ssc_user, fortify_ssc_password), headers=fortify_ssc_api_request_headers, verify=False, timeout=900)
        ...
        if fortify_ssc_token_deletion_response.status_code == 200:
            ...
            # Attempts to handle any ValueError that occurs if the response is not JSON
            except ValueError:
                print(fortify_ssc_token_deletion_response.text)

            print("")

            print(colored(f"The token with ID '{fortify_ssc_token_id}' " f"was successfully deleted!", "green"))

            logging.info(f"The token with ID '{fortify_ssc_token_id}' " f"was successfully deleted!")
        ...
        # If the request was not successful, it prints an error message and logs the failure
            ...
            print(colored(f"The status code from the request of Fortify SSC token deletion is: " f"{fortify_ssc_token_deletion_response.status_code}", "red"))
            ...
            print(colored(f"Failed to delete the token. " f"Status code: {fortify_ssc_token_deletion_response.status_code} " f"- {fortify_ssc_token_deletion_response.text}\n", "red"))
 
            logging.error(f"Failed to delete the token. " f"Status code: {fortify_ssc_token_deletion_response.status_code} " f"- {fortify_ssc_token_deletion_response.text}")

    # Attempts to handle any exceptions that occur during the request
    except requests.exceptions.RequestException as e:
