...
if [[ -d "$EDIRECTORY_LDAP_BACKUP_DIR" ]]; then
    ...
    mkdir -p $EDIRECTORY_LDAP_INSTALLATION_DIR                          
    mkdir -p $IDENTITYCONSOLE_LDAP_INSTALLATION_DIR
    ...
    # Loads the Docker Image for EDirectory version x.x.x
    ...
    # Loads the Docker Image for EDirectory API version xx.x
    ...
    # Loads the Docker Image for IdentityConsole version xx.x
    ...
    # If one or more of the LDAP Docker Images are missing in the Private Docker Registry, it tags and pushes the Docker Images to the Private Docker Registry
        ...
        # Step 9: Shows all the repositories in the Docker Private Registry
